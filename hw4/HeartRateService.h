...
public:
     ...
     */
    HeartRateService(BLE &_ble, uint16_t hrmCounter, uint16_t hrmCounter1, uint16_t hrmCounter2, uint16_t hrmCounter3, BodySensorLocation location) :
        ble(_ble),
        valueBytes(hrmCounter),
        valueBytes1(hrmCounter1),
        valueBytes2(hrmCounter2),
        valueBytes3(hrmCounter3),
        hrmRate(
        ...
        hrmRate1(
            ...
            valueBytes1.getPointer(),
            valueBytes1.getNumValueBytes(),
            HeartRateValueBytes::MAX_VALUE_BYTES,
            GattCharacteristic::BLE_GATT_CHAR_PROPERTIES_NOTIFY | GattCharacteristic::BLE_GATT_CHAR_PROPERTIES_READ
        ),
        hrmRate2(
            GattCharacteristic::UUID_HID_INFORMATION_CHAR,
            valueBytes2.getPointer(),
            valueBytes2.getNumValueBytes(),
            HeartRateValueBytes::MAX_VALUE_BYTES,
            GattCharacteristic::BLE_GATT_CHAR_PROPERTIES_NOTIFY | GattCharacteristic::BLE_GATT_CHAR_PROPERTIES_READ
        ),
        hrmRate3(
            GattCharacteristic::UUID_HUMIDITY_CHAR,
            valueBytes3.getPointer(),
            valueBytes3.getNumValueBytes(),
            HeartRateValueBytes::MAX_VALUE_BYTES,
            GattCharacteristic::BLE_GATT_CHAR_PROPERTIES_NOTIFY | GattCharacteristic::BLE_GATT_CHAR_PROPERTIES_READ
        ),
        hrmLocation(
    ...
    void updateHeartRate1(uint16_t hrmCounter) {
        valueBytes1.updateHeartRate(hrmCounter);
        ble.gattServer().write(
            hrmRate1.getValueHandle(),
            valueBytes1.getPointer(),
            valueBytes1.getNumValueBytes()
        );
    }

    void updateHeartRate2(uint16_t hrmCounter) {
        valueBytes2.updateHeartRate(hrmCounter);
        ble.gattServer().write(
            hrmRate2.getValueHandle(), 
            valueBytes2.getPointer(),
            valueBytes2.getNumValueBytes()
        );
    }

    void updateHeartRate3(uint16_t hrmCounter) {
        valueBytes3.updateHeartRate(hrmCounter);
        ble.gattServer().write(
            hrmRate3.getValueHandle(),
            valueBytes3.getPointer(),
            valueBytes3.getNumValueBytes()
        );
    }
...
protected:
    ...
    void setupService() {
        ...
        GattCharacteristic *charTable1[] = {
            &hrmRate1,
            &hrmLocation
        };
        GattService hrmService1(
            GattService::UUID_NEXT_DST_CHANGE_SERVICE,
            charTable1,
            sizeof(charTable) / sizeof(charTable[0])
        );

        GattCharacteristic *charTable2[] = {
            &hrmRate2,
            &hrmLocation
        };
        GattService hrmService2(
            GattService::UUID_IMMEDIATE_ALERT_SERVICE,
            charTable2,
            sizeof(charTable) / sizeof(charTable[0])
        );

        GattCharacteristic *charTable3[] = {
            &hrmRate3,
            &hrmLocation
        };
        GattService hrmService3(
            GattService::UUID_LINK_LOSS_SERVICE,
            charTable3,
            sizeof(charTable) / sizeof(charTable[0])
        );

        ble.gattServer().addService(hrmService);
        ble.gattServer().addService(hrmService1);
        ble.gattServer().addService(hrmService2);
        ble.gattServer().addService(hrmService3);
    }
    ...
    struct HeartRateValueBytes {
        ...
        void updateHeartRate(uint16_t hrmCounter)
        {
            if (hrmCounter <= 255) {
                ...
                valueBytes[FLAGS_BYTE_INDEX] = (uint8_t)(hrmCounter >> 8);
                valueBytes[FLAGS_BYTE_INDEX + 1] = (uint8_t)(hrmCounter & 0xFF);
            }
        }
...
protected:
    BLE &ble;
    HeartRateValueBytes valueBytes, valueBytes1, valueBytes2, valueBytes3;
    GattCharacteristic hrmRate, hrmRate1, hrmRate2, hrmRate3;
